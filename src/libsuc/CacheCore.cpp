...
CacheGeneric<State, Addr_t, Energy> *CacheGeneric<State, Addr_t, Energy>::createFA(const char *section, const char *append, const char *format, ...)
{
    ...
    if(SescConf->isGT(section, size, 0) &&
        ...
        a = s/b;
        cache = create(s, a, b, u, pStr, sk);
        printf("Creating dummy FA cache size %d, asso %d, bsize %d, addrUnit %d, repl %s\n",s,a,b,u,pStr);
    } else {
        // this is just to keep the configuration going,
        // sesc will abort before it begins
        cache = new CacheAssoc<State, Addr_t, Energy>(2,
...
typename CacheAssoc<State, Addr_t, Energy>::Line *CacheAssoc<State, Addr_t, Energy>::findLinePrivate(Addr_t addr)
{
    ...
    // if(numLines == assoc){
    //     theSet = &content[0];
    //     //printf("findLinePrivate: this is FA, theSet=%d\n",int(theSet));
    // }
...
/// Dead code for Proj2!!!
    if (lineFree == 0) {///no hit; didnt find invalid, and all are locked. And ignore lock
        ...
        } else if (policy == NXLRU) {///NXLRU for the "All lines are valid and locked" case --this only works for ignoreLock case!
            lineFree = setEnd-2;
...
template<class State, class Addr_t, bool Energy>
typename CacheAssoc<State, Addr_t, Energy>::Line*
CacheAssoc<State, Addr_t, Energy>::findLineFA(Addr_t addr)
{
    Addr_t tag = calcTag(addr);
